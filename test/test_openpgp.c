...
static void test_x25519_public_key_encoding(void **state) {
  ...
  // Public key derived from the imported scalar after the RFC 7748 §5 clamp.
  // Computed offline against the canonical X25519(priv, 9) reference; the
  // earlier expectation was anchored to a buggy import path that skipped both
  // the LE→BE swap and the clamp.
  uint8_t expected[] = {
      0x7F, 0x49, 0x22, 0x86, 0x20, 0xDE, 0x5C, 0x41, 0x67, 0xE6, 0x5A, 0xB9, 0x70, 0x38, 0x49, 0x77, 0x01, 0x90, 0xDD,
      0xDB, 0x09, 0x91, 0x49, 0x49, 0xB0, 0xF4, 0x3F, 0xE4, 0xB6, 0xF3, 0xD7, 0xB3, 0x70, 0x7D, 0xE2, 0xDB, 0x75,
  };

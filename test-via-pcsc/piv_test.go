...
func (app *PIVApplet) Authenticate() {
	...
	chal, code, err := app.Send([]byte{0x00, 0x87, 0x0A, 0x9B, 0x04, 0x7C, 0x02, 0x81, 0x00, 0x00})
	So(err, ShouldBeNil)
	So(code, ShouldEqual, 0x9000)
	...
	_, code, err = app.Send(append([]byte{0x00, 0x87, 0x0A, 0x9B, byte(l + 4), 0x7C, byte(l + 2), 0x82, byte(l)}, rsp...))
	So(err, ShouldBeNil)
	So(code, ShouldEqual, 0x9000)
}

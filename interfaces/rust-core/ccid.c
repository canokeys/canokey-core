...
#define CCID_INVALID_COMMAND 0xffu
...
uint8_t CCID_OutEvent(uint8_t *data, uint8_t length) {
  if (reset_pending || phase >= CCID_QUEUED || !length) return 0;
  if (phase == CCID_IDLE) {
    if (length < CCID_CMD_HEADER_SIZE) return 0;
    expected = ccid_get_le32(data + CCID_LENGTH);
    if (expected > FRAME_CAPACITY) {
      memcpy(request, data, CCID_CMD_HEADER_SIZE);
      received = CCID_CMD_HEADER_SIZE;
      expected = CCID_CMD_HEADER_SIZE;
      // Oversized CCID frames are transport errors, not APDU commands.
      request[CCID_MESSAGE_TYPE] = CCID_INVALID_COMMAND;
      phase = CCID_QUEUED;
      return 0;
    }
    expected += CCID_CMD_HEADER_SIZE;
...
#endif
      int32_t n = ck_core_exchange(1, request + CCID_CMD_HEADER_SIZE, expected - CCID_CMD_HEADER_SIZE,
                                   response + CCID_CMD_HEADER_SIZE, sizeof(response) - CCID_CMD_HEADER_SIZE);
#if defined(RUST_CORE_OPENPGP) || defined(RUST_CORE_PIV)

...
static uint8_t openpgp_out_buf[64];
...
uint8_t USBD_CCID_Init(USBD_HandleTypeDef *pdev) {
  ...
  if (EP_OUT(openpgp) != 0xFF) {
    USBD_LL_OpenEP(pdev, EP_IN(openpgp), USBD_EP_TYPE_BULK, EP_SIZE(openpgp));
    USBD_LL_OpenEP(pdev, EP_OUT(openpgp), USBD_EP_TYPE_BULK, EP_SIZE(openpgp));
  }
  CCID_Init();
  USBD_LL_PrepareReceive(pdev, EP_OUT(ccid), ccid_out_buf, EP_SIZE(ccid));
  if (EP_OUT(openpgp) != 0xFF) {
    USBD_LL_PrepareReceive(pdev, EP_OUT(openpgp), openpgp_out_buf, EP_SIZE(openpgp));
  }
  return 0;
}
...
uint8_t USBD_CCID_DataOut(USBD_HandleTypeDef *pdev, uint8_t idx) {
  uint8_t addr = idx == IDX_CCID ? EP_OUT(ccid) : EP_OUT(openpgp);
  uint8_t size = idx == IDX_CCID ? EP_SIZE(ccid) : EP_SIZE(openpgp);
...
uint8_t CCID_Response_SendData(USBD_HandleTypeDef *pdev, const uint8_t *buf, uint16_t len, uint8_t idx,
  ...
  if (pdev->dev_state == USBD_STATE_CONFIGURED) {
    ...
    uint8_t addr = idx == IDX_CCID ? EP_OUT(ccid) : EP_OUT(openpgp);
    uint8_t ep_size = idx == IDX_CCID ? EP_SIZE(ccid) : EP_SIZE(openpgp);

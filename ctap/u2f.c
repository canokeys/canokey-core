...
int u2f_register(const CAPDU *capdu, RAPDU *rapdu) {
  ...
  ecc_generate(ECC_SECP256R1, handle + U2F_APPID_SIZE, handle + U2F_APPID_SIZE + U2F_EC_KEY_SIZE);

  uint8_t key_buf[U2F_EC_KEY_SIZE + U2F_EC_PUB_KEY_SIZE + U2F_SECRET_KEY_SIZE];
  int err = read_file(KEY_FILE, key_buf, 0, sizeof(key_buf));
  if (err < 0) return err;
  ...
  memcpy(resp->pubKey.x, handle + U2F_APPID_SIZE + U2F_EC_KEY_SIZE, U2F_EC_PUB_KEY_SIZE);
  // KEY HANDLE LENGTH (1)
  ...
  // CERTIFICATE (var)
  int cert_len = read_file(CERT_FILE, resp->keyHandleCertSig + U2F_KH_SIZE, 0, U2F_MAX_ATT_CERT_SIZE);
  ...
  size_t signature_len = ecdsa_sig2ansi(handle + 32, resp->keyHandleCertSig + U2F_KH_SIZE + cert_len);
...
int u2f_authenticate(const CAPDU *capdu, RAPDU *rapdu) {
  ...
  if (req->keyHandleLen != U2F_KH_SIZE) EXCEPT(SW_WRONG_LENGTH);
  ...
  if (memcmp(req->appId, req->keyHandle, U2F_APPID_SIZE) != 0) EXCEPT(SW_WRONG_DATA);
  ...
  ecdsa_sign(ECC_SECP256R1, req->keyHandle + U2F_APPID_SIZE, req->appId, resp->sig);
...
int u2f_install_private_key(const CAPDU *capdu, RAPDU *rapdu) {
  ...
  if (LC != U2F_EC_KEY_SIZE) EXCEPT(SW_WRONG_LENGTH);

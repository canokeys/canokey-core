...
uint8_t parse_public_key_credential_descriptor(CborValue *lst, uint8_t *id) {
  CborValue arr;
  size_t size;
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    size_t sz = sizeof(KeyHandle);
    if (id) {
      ret = cbor_value_copy_byte_string(&val, id, &sz, NULL);
      CHECK_PARSING(ret);
    }
...
uint8_t parse_make_credential(CTAP_makeCredential *mc, const uint8_t *buf, size_t len) {
  CborParser parser;
  CborValue it, map;
  size_t map_length;
  int key, pinProtocol;
  ...
  for (size_t i = 0; i < map_length; ++i) {
    ...
    case MC_excludeList:
      ...
      ret = parse_public_key_credential_descriptor(&map, NULL);
      if (ret != 0) return ret;
...
uint8_t parse_get_assertion(CTAP_getAssertion *ga, const uint8_t *buf, size_t len) {
  CborParser parser;
  CborValue it, map;
  size_t map_length;
  int key, pinProtocol;

...
Output segments (concatenated at runtime in C):
  ...
  3. cbor_gi_alg_sm2[]    — SM2 algorithm entry
...
def main():
    ...
    parser.add_argument("--sm2-algo-id", type=int, default=-54)
    ...
    total = len(prefix) + 1 + len(alg_base) + len(alg_sm2) + len(suffix)
    ...
    with open(args.output, 'w') as f:
        ...
        f.write(f"//\n")
        f.write(f"// Total response size: {total} bytes\n\n")

...
def encode_transports(enable_nfc):
    transports = ["usb"]
    if enable_nfc:
        transports.insert(0, "nfc")
    return encode_array_header(len(transports)) + b"".join(encode_text(name) for name in transports)
...
def build_segments(c, enable_nfc):
    segments = {}
    ...
    after_versions += encode_transports(enable_nfc)
    ...
    suffix += encode_transports(enable_nfc)
...
def main():
    ...
    segments = build_segments(consts, args.enable_nfc)

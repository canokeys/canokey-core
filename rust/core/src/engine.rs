// SPDX-License-Identifier: Apache-2.0
#![forbid(unsafe_code)]
use canokey_protocol::{
    apdu,
    response::{ResponsePlan, StatusWord},
};

use crate::services::Platform;

#[derive(Clone, Copy)]
pub enum Reply {
    Status(StatusWord),
    Data(u32),
}

#[derive(Clone, Copy)]
struct Pending {
    total: u32,
    offset: u32,
    sw: StatusWord,
}

pub struct Core {
    #[cfg(feature = "pass")]
    output: crate::output::Output,
    owner: u8,
    selected: bool,
    #[cfg(feature = "pass")]
    grants: crate::admin::Grants,
    chain: apdu::CommandChain,
    pending: Option<Pending>,
    #[cfg(feature = "pass")]
    registry: crate::registry::Registry,
}

impl Default for Core {
    fn default() -> Self {
        Self::new()
    }
}

impl Core {
    pub const fn new() -> Self {
        Self {
            #[cfg(feature = "pass")]
            output: crate::output::Output::new(),
            owner: 0,
            selected: false,
            #[cfg(feature = "pass")]
            grants: crate::admin::Grants { admin: false },
            chain: apdu::CommandChain::new(),
            pending: None,
            #[cfg(feature = "pass")]
            registry: crate::registry::Registry::new(),
        }
    }
    pub const fn applet_count() -> u8 {
        if cfg!(feature = "oath") {
            2
        } else if cfg!(feature = "pass") {
            1
        } else {
            0
        }
    }

    pub fn install(&mut self, platform: &mut dyn Platform) -> Result<(), StatusWord> {
        self.reset(platform);
        #[cfg(feature = "pass")]
        self.registry.install(platform)?;
        Ok(())
    }

    /// Called by the transport on disconnect/reset or an explicitly authorized
    /// owner handoff. Timeout policy belongs to the interface, not an applet.
    pub fn reset(&mut self, platform: &mut dyn Platform) {
        #[cfg(feature = "pass")]
        self.output.reset(platform);
        self.owner = 0;
        self.selected = false;
        self.pending = None;
        self.chain.reset();
        #[cfg(feature = "pass")]
        {
            self.registry.reset(platform);
            self.grants.admin = false;
        }
        #[cfg(not(feature = "pass"))]
        let _ = platform;
    }

    /// Consume the complete short transport frame. Its bytes expire at return.
    /// ISO command chains are consumed into the selected applet's bounded
    /// semantic fields, not a core-wide full-command buffer.
    pub fn receive(&mut self, owner: u8, frame: &[u8], platform: &mut dyn Platform) -> Reply {
        if owner == 0 || (self.owner != 0 && self.owner != owner) {
            return Reply::Status(StatusWord::CONDITIONS_NOT_SATISFIED);
        }
        self.owner = owner;
        let command = match apdu::parse(frame) {
            Ok(command) => command,
            Err(_) => {
                self.pending = None;
                self.chain.reset();
                #[cfg(feature = "pass")]
                self.registry.cancel_command(platform);
                return Reply::Status(StatusWord::WRONG_LENGTH);
            }
        };
        // The parser supports extended format; this minimal transport profile
        // admits short frames and ISO chaining only. No implicit FIDO exception.
        if command.info.extended {
            self.pending = None;
            self.chain.reset();
            #[cfg(feature = "pass")]
            self.registry.cancel_command(platform);
            return Reply::Status(StatusWord::WRONG_LENGTH);
        }
        let header = command.info.header;
        // Missing Le requests the bounded response for this documented profile.
        let le = command.info.le.unwrap_or(256);
        if header.is_get_response() {
            self.chain.reset();
            #[cfg(feature = "pass")]
            self.registry.cancel_command(platform);
            return if self.pending.is_some() {
                Reply::Data(le)
            } else {
                Reply::Status(StatusWord::COMMAND_NOT_ALLOWED)
            };
        }
        #[cfg(feature = "pass")]
        if self.output.busy() {
            return Reply::Status(StatusWord::CONDITIONS_NOT_SATISFIED);
        }
        self.pending = None;
        if header.cla == 0 && header.ins == 0xa4 && header.p1 == 4 {
            self.chain.reset();
            #[cfg(feature = "pass")]
            {
                self.registry.cancel_command(platform);
                if header.p2 == 0 {
                    match self
                        .registry
                        .select(command.data, &mut self.grants, platform)
                    {
                        Ok(total) => {
                            self.selected = true;
                            return self.reply(total, StatusWord::SUCCESS, le);
                        }
                        Err(sw) => return Reply::Status(sw),
                    }
                }
            }
            return Reply::Status(if header.p2 == 0 {
                StatusWord::FILE_NOT_FOUND
            } else {
                StatusWord::WRONG_P1P2
            });
        }
        if !self.selected {
            return Reply::Status(StatusWord::FILE_NOT_FOUND);
        }
        #[cfg(feature = "pass")]
        {
            if !self.registry.accepts_cla(header) {
                self.chain.reset();
                self.registry.cancel_command(platform);
                return Reply::Status(StatusWord::CLA_NOT_SUPPORTED);
            }
            // The registry supplies the selected adapter's bounded consumer.
            let step = match self
                .chain
                .accept(command.info, self.registry.command_capacity())
            {
                Ok(step) => step,
                Err(_) => {
                    self.registry.cancel_command(platform);
                    return Reply::Status(StatusWord::WRONG_LENGTH);
                }
            };
            if step.restarted {
                self.registry.cancel_command(platform);
            }
            if let Err(sw) = self.registry.consume(command.data) {
                self.chain.reset();
                self.registry.cancel_command(platform);
                return Reply::Status(sw);
            }
            if !step.last {
                return Reply::Status(StatusWord::SUCCESS);
            }
            let result = self
                .registry
                .finish(header.unchained(), le, &mut self.grants, platform);
            if header.ins == 0x50 {
                self.output.reset(platform);
            }
            match result {
                Ok((total, sw)) => self.reply(total, sw, le),
                Err(sw) => Reply::Status(sw),
            }
        }
        #[cfg(not(feature = "pass"))]
        {
            let _ = platform;
            Reply::Status(StatusWord::INS_NOT_SUPPORTED)
        }
    }

    #[cfg(feature = "pass")]
    fn reply(&mut self, total: u32, sw: StatusWord, le: u32) -> Reply {
        if total == 0 {
            Reply::Status(sw)
        } else {
            self.pending = Some(Pending {
                total,
                offset: 0,
                sw,
            });
            Reply::Data(le)
        }
    }
    /// Produce payload plus SW. Called only after the input borrow ends, so a C
    /// interface can use the same transport buffer for RX and TX. Streamed data
    /// is regenerated from stable Rust applet state; no saved-tail C machinery.
    pub fn transmit(&mut self, reply: Reply, output: &mut [u8]) -> Result<usize, StatusWord> {
        if output.len() < 2 {
            return Err(StatusWord::WRONG_LENGTH);
        }
        #[cfg(feature = "pass")]
        let mut len = 0;
        #[cfg(not(feature = "pass"))]
        let len = 0;
        let sw = match reply {
            Reply::Status(sw) => sw,
            Reply::Data(le) => {
                let Some(pending) = self.pending else {
                    return Self::status_only(output, StatusWord::COMMAND_NOT_ALLOWED);
                };
                let capacity = (output.len() - 2).min(256) as u32;
                let plan =
                    ResponsePlan::new(pending.total, pending.offset, le.min(capacity), pending.sw)?;
                #[cfg(feature = "pass")]
                {
                    len = plan.length as usize;
                    if self
                        .registry
                        .read_response(pending.offset as usize, &mut output[..len])
                        .is_err()
                    {
                        self.pending = None;
                        return Self::status_only(output, StatusWord::UNABLE_TO_PROCESS);
                    }
                }
                self.pending = if plan.complete {
                    None
                } else {
                    Some(Pending {
                        offset: plan.next,
                        ..pending
                    })
                };
                plan.sw
            }
        };
        output[len..len + 2].copy_from_slice(&sw.bytes());
        Ok(len + 2)
    }

    fn status_only(output: &mut [u8], sw: StatusWord) -> Result<usize, StatusWord> {
        output[..2].copy_from_slice(&sw.bytes());
        Ok(2)
    }

    #[cfg(feature = "pass")]
    pub fn sample_output(
        &mut self,
        pressed: bool,
        now: u32,
        ready: bool,
        platform: &mut dyn Platform,
    ) -> Option<u8> {
        if self.registry.take_presence() {
            self.output.reset(platform);
        }
        if self.chain.active() || self.pending.is_some() {
            // No queued secret-output gesture may outlive an APDU lease.
            self.output.inhibit(pressed, platform);
            return None;
        }
        self.output
            .sample(pressed, now, ready, &self.registry, platform)
    }
    #[cfg(feature = "pass")]
    pub fn touch(
        &self,
        index: u8,
        output: &mut [u8],
        platform: &mut dyn Platform,
    ) -> Result<usize, StatusWord> {
        {
            let _ = platform;
            self.registry.touch(index, output, platform)
        }
    }
    #[cfg(feature = "pass")]
    pub fn challenge(
        &self,
        index: u8,
        challenge: &[u8],
        output: &mut [u8; 20],
        platform: &mut dyn Platform,
    ) -> Result<(), StatusWord> {
        self.registry.challenge(index, challenge, output, platform)
    }
}
